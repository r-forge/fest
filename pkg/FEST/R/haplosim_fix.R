haplosim.fix <- function (n, hap, which.snp = NULL, seed = NULL, force.polym = TRUE, 
                          summary = TRUE) 
{
  if (length(seed) > 0) 
    set.seed(seed)
  ...
  pol.flag <- y.freqs$all.polym
  if (n > 1 & force.polym & !pol.flag) {
    while (!pol.flag) {
      A <- mvrnorm(n, mu = rep(0, nloci), Sigma = hap$cov[indexset, 
                                            indexset], tol = 1e-06, empirical = FALSE)
      if (n == 1) 
        A <- array(A, dim = c(1, nloci))
      for (i in 1:n) for (j in 1:nloci) {
        if (A[i, j] <= quants[j]) 
          y[i, j] <- 0
        else y[i, j] <- 1
      }
